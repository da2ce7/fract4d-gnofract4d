...
fractThreadFunc::antialias(int x, int y)
{
    ...
    // top left
    pf->calc(pos, f->maxiter,nNoPeriodIters,x,y,1,&ptmp,&p,NULL); 
    pixel_r_val += ptmp.r;
    pixel_g_val += ptmp.g;
    pixel_b_val += ptmp.b;
    pos+=ff->delta_aa_x;
    ...
    // top right
    pf->calc(pos, f->maxiter,nNoPeriodIters,x,y,2,&ptmp,&p,NULL); 
    pixel_r_val += ptmp.r;
    pixel_g_val += ptmp.g;
    pixel_b_val += ptmp.b;
    ...
    // bottom left
    pf->calc(pos, f->maxiter,nNoPeriodIters,x,y,3,&ptmp,&p,NULL); 
    pixel_r_val += ptmp.r;
    pixel_g_val += ptmp.g;
    pixel_b_val += ptmp.b;
    pos+= ff->delta_aa_x;
    ...
    // bottom right
    pf->calc(pos, f->maxiter,nNoPeriodIters,x,y,4,&ptmp,&p,NULL); 
    pixel_r_val += ptmp.r;
    pixel_g_val += ptmp.g;
    pixel_b_val += ptmp.b;
...
fractThreadFunc::pixel(int x, int y,int w, int h)
{
    ...
    pf->calc(pos, f->maxiter,periodGuess(), x,y,0,&pixel,&iter,buf); 
    periodSet(&iter);
    im->setIter(x,y,iter);
    ...
    if(f->auto_deepen && k++ % ff->AUTO_DEEPEN_FREQUENCY == 0)
    {
        ...
        pf->calc(pos, f->maxiter*2,periodGuess()*2,x,y,-1,NULL,&i,buf);
