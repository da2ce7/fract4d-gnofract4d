...
#
#   FractalDirector.py: main window.
#
...
import DirectorBean,FCTGen,PNGGen,AVIGen,DlgAdvOpt
...
class DirectorDialog(dialog.T):
	...
	def check_sanity(self):
		try:
			...
			if self.dir_bean.get_fct_dir()=="":
				error_dlg = gtk.MessageDialog(self.window,gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
			...
			if self.dir_bean.keyframes_count()<1:
				...
				error_dlg.destroy()
				return -1
			#chech if any fct files are in temp fct dir
	...
	#first we generate .fct files, then. png files and list and then .avi
	def generate(self, widget, data=None):
		if self.check_sanity()!=0:
			return
		fct_gen=FCTGen.FCTGeneration(self.dir_bean,self.window)
		res=fct_gen.show()
		if res==1:
			error_dlg = gtk.MessageDialog(self.window,gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
			...
			dlg = gtk.MessageDialog(self.window,gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
					gtk.MESSAGE_INFO, gtk.BUTTONS_OK,
					".fct files created. Now I'll try creating images. Press OK to continue")
			dlg.run()
			dlg.destroy()
		else:
			return
		png_gen=PNGGen.PNGGeneration(self.dir_bean,self.window)
		res=png_gen.show()
		if res==1:
			error_dlg = gtk.MessageDialog(self.window,gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
					gtk.MESSAGE_INFO, gtk.BUTTONS_OK,
			...
			dlg = gtk.MessageDialog(self.window,gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
					gtk.MESSAGE_INFO, gtk.BUTTONS_OK,
					".png files created. Now I'll try creating avi. Press OK to continue")
			dlg.run()
			dlg.destroy()
		else:
			return
		avi_gen=AVIGen.AVIGeneration(self.dir_bean,self.window)
		res=avi_gen.show()
		if res==1:
			error_dlg = gtk.MessageDialog(self.window,gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
					gtk.MESSAGE_INFO, gtk.BUTTONS_OK,
	...
	def adv_opt_clicked(self,widget,data=None):
		if self.current_select==-1:
			return
		dlg=DlgAdvOpt.DlgAdvOptions(self.current_select,self.dir_bean,self.window)
		res=dlg.show()
	...
	def load_configuration(self,widget,data=None):
		...
		if cfg!="":
			...
			#before we overwrite our DirecotryBean and clear keyframe list
			try:
				inp=open(cfg,"r")
	...
	def about(self,widget,data=None):
		...
		about.set_copyright("Branko Kokanovic 2006©")
		about.set_comments("Utility to create fractal videos")
		#about.set_licence("GPL")
	...
	def __init__(self, main_window, f):
		...
		#self.window.set_size_request(770,730)
		...
		self.menu_items = (
			...
			( "/File/_Load configuration",     "<control>O", self.load_configuration, 0, None ),
			( "/File/_Save configuration",    "<control>S", self.save_configuration, 0, None ),
		...
		self.tbl_dirs=gtk.Table(2,3,False)
