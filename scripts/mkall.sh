...
mv dist/gnofract4d-3.10-1.$ARCH.rpm dist/gnofract4d-python25-3.9-1.$ARCH.rpm 
...
scripts/mkrpm.sh $BUILD_PYTHON $MIN $MAX
mv dist/gnofract4d-3.10-1.$ARCH.rpm dist/gnofract4d-python24-3.9-1.$ARCH.rpm 
...
pushd dist
mkisofs -J -R -o gf4d.iso *3.10*.gz *3.9*.rpm *3.9*.deb
popd dist
